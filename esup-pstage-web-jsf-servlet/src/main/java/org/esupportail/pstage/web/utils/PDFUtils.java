...
public final class PDFUtils {
	...
	public static void exportPDF(final String fileNameXml,
		...
		try {
			...
			boolean b = f1.delete();
			if (b == false){
				LOGGER.error(" Erreur lors de la tentative de suppression du fichier " + fileNamePdf);
			}
	...
	public static void exportPDFinDirectory(final String fileNameXml,
		...
		byte[] arrayPDF;
		try {
			arrayPDF = PDFUtils.transformXMLPDF(f1,
					new File(directory + fileNameXsl),
					directory);
		}  catch (TransformerException e) {
			LOGGER.error(" Probleme de transformation pdf = " + fileNamePdf + "exception : " + e);
		...
		try (FileOutputStream fos = new FileOutputStream(someFile+fileNamePdf)){
	...
	public static ZipOutputStream preparePDFinZip(final String fileNameXml,
		...
		try {
			...
			boolean b = f1.delete();
			if (b == false) {
				LOGGER.error(" Erreur lors de la tentative de suppression du fichier " + fileNameXsl);
			}
		} catch (TransformerException e) {
	...
	public static void setPDFfilesInZip(final List<String> fileNameXmlList,
		...
		try(OutputStream out= new FileOutputStream(chemin)) {
	...
	private static byte[] transformXMLPDF(final File xml,
		...
		// Setup logger
		org.apache.avalon.framework.logger.Logger logger = new ConsoleLogger(ConsoleLogger.LEVEL_DISABLED);
